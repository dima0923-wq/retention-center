  ...
  static async create(data: WebhookCreateInput) {
    ...
    const webhook = await prisma.webhook.create({
      data: {
        ...
        slug,
        type: data.type,
        sourceLabel: data.sourceLabel ?? data.type.toUpperCase(),
        ...
        fieldMapping: data.fieldMapping ? JSON.stringify(data.fieldMapping) : "{}",
      },
      include: { campaign: true, sequence: true },
    });
  ...
  static async update(id: string, data: WebhookUpdateInput) {
    ...
    const webhook = await prisma.webhook.update({
      ...
      data: {
        ...
        ...(data.config !== undefined && { config: data.config ? JSON.stringify(data.config) : "{}" }),
